import React from "react";
import {
  BsFillTelephoneInboundFill,
  BsLinkedin,
  BsFacebook,
  BsInstagram,
} from "react-icons/bs";
import { MdMarkEmailRead } from "react-icons/md";

const Footer = () => {
  return (
    <div className="w-full h-auto flex flex-col items-center bg-orange-100 justify-center">
      <section
        className="w-full p-6 px-10 mt-8 mb-8 md:px-16 lg:px-20 py-4 relative
        before:w-full before:h-[1px] before:left-0 before:top-0 
        before:bg-slate-600 before:content before:absolute
        after:w-full after:h-[1px] after:left-0 after:bottom-0 
        after:bg-slate-600 after:content after:absolute"
      >
        <div
          className="flex flex-col w-full items-center justify-between
        px-4 md:px-0 max-w-6xl mx-auto"
        >
          <div className="w-full flex items-center justify-center">
            <p
              className="text-2xl mb-12 font-semibold capitalize text-[#2e2e2e] 
              relative before:absolute before:rounded-lg before:content before:w-20
              before:h-1 before:-bottom-2 before:left-11 
              before:bg-gradient-to-tr from-orange-400 to-orange-600
              transition-all ease-in-out duration-100 "
            >
              Our <span className="text-slate-500">contacts</span>
            </p>
          </div>

          <div
            className="w-full flex flex-col lg:flex-row items-center
          justify-between mb-10"
          >
            <div
              className="w-full flex items-center justify-center group
            cursor-pointer py-2"
            >
              <BsFillTelephoneInboundFill
                className="text-base md:text-lg text-slate-400
                group-hover:text-slate-500"
              />
              <p
                className="text-base md:text-lg text-slate-400 px-2 
                group-hover:text-slate-500"
              >
                +91 7210949505{" "}
              </p>
            </div>
            <div
              className="w-full flex items-center 
            justify-center cursor-pointer group py-2"
            >
              <MdMarkEmailRead
                className="text-base md:text-lg 
              text-slate-400 group-hover:text-slate-500"
              />
              <p
                className="text-base md:text-lg text-slate-400 
              px-2 group-hover:text-slate-500"
              >
                jha04527@gmail.com
              </p>
            </div>
            <div className="w-full flex items-center justify-center gap-4 py-2">
              <BsLinkedin
                className="text-lg text-slate-400 hover:text-slate-500 
              duration-100 transition-all ease-in-out cursor-pointer"
              />
              <BsFacebook
                className="text-lg text-slate-400 hover:text-slate-500
               duration-100 transition-all ease-in-out cursor-pointer"
              />
              <BsInstagram
                className="text-lg text-slate-400 hover:text-slate-500 
              duration-100 transition-all ease-in-out cursor-pointer"
              />
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Footer;
